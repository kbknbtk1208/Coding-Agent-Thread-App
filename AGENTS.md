...
- ユーザーから明示的な指示が無い限りは余計なlabelなどの配置は避け、情報量を必要最低限にとどめる
- カード型UIのネストは極力避ける。情報のまとまりは要素の集約や余白によって再現する
- ユーザーによるクリックを想定する要素（ボタンやメニューアイテム、アコーディオンの開閉ボタンなど）にはcursor:pointerを付与する
