...
import type {
  GraphAnalysisEvent,
  Poc3ArchivedRemoteThread,
} from '../../../../shared/poc3-contracts/graph-review-ipc';

export function useArchivedRemoteThreads(reviewWorkspaceId: string) {
  const [threads, setThreads] = useState<Poc3ArchivedRemoteThread[]>([]);

  const load = useCallback(async () => {
    const result = await window.poc3GraphReviewApi.listArchivedRemoteThreads({
      reviewWorkspaceId,
    });
